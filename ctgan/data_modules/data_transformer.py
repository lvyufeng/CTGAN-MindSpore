...
class DataTransformer(object):
    ...
    def generate_tensors(self):
        ...
        - `cond_tensor`: it is used in :func:`ctgan.losses.conditional_loss`
          ...
          :meth:`ctgan.data_modules.DataTransformer.fit`.
        """
        if self.output_info_list is None:
        ...
        for column_info in self.output_info_list:
            for item in column_info:
                ed_idx = st_idx + item.dim
                if len(column_info) == 1:
                    ed_c = st_c + item.dim
                    cond_info.append((st_idx, ed_idx, st_c, ed_c, i))
                    st_c = ed_c
                    i += 1

                output_info.append((st_idx, ed_idx, int(item.activation_fn == 'softmax')))
                st_idx = ed_idx
